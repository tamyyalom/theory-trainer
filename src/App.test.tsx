...
describe('App', () => {
  ...
  it('shows home screen actions after loading question bank', async () => {
    ...
    render(<App />)
    expect(await screen.findByText('לימוד לפי נושא')).toBeInTheDocument()
  ...
  it('navigates from home to learn mode', async () => {
    ...
    render(<App />)
    await user.click(await screen.findByText('לימוד לפי נושא'))
    expect(await screen.findByText('לימוד לפי נושא')).toBeInTheDocument()
    expect(screen.getByText('שאלה לבדיקה')).toBeInTheDocument()
